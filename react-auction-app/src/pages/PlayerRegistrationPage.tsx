...
export default function PlayerRegistrationPage() {
  ...
  const saveEditedPhoto = async (file: File) => {
    const editedUrl = await uploadFileToStorage(file, `media/registrations/${tenantSlug}/edited-${Date.now()}`);
    setPhotoUrl(editedUrl);
    setPhotoPreview(editedUrl);
    setPhotoFile(file);
  };
